...
class RSNAClassifierDataset(Dataset):
    """
    input_mode:
      - plain      -> uses plain_path (full CXR resized to img_size)
...
def main():
    ...
    parser.add_argument("--normalize", choices=["imagenet", "cxr", "none"],
                        default="imagenet",
                        help="from-scratch icin 'cxr' onerilir")
    parser.add_argument("--augmentation", choices=["mild", "strong"],
                        default="mild",
                        help="from-scratch icin 'strong' onerilir")
    parser.add_argument("--scheduler", choices=["plateau", "cosine"],
                        default="plateau",
                        help="from-scratch icin 'cosine' onerilir")
    ...
    train_ds = RSNAClassifierDataset(args.train_csv, args.input_mode,
                                     is_train=True, **ds_kwargs)
    val_ds = RSNAClassifierDataset(args.val_csv, args.input_mode,
                                   is_train=False, **ds_kwargs)
    test_ds = RSNAClassifierDataset(args.test_csv, args.input_mode,
                                    is_train=False, **ds_kwargs)
